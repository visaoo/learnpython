...
def init_game(): # type: ignore
    ...
    while tentativas > 0: # Enquanto tentativas for maior que 0 o loop continuará.
        ...
        acertou = numero_escolhido == get_codiguin
        print(type(numero_escolhido), type(get_codiguin))
        if acertou:
            print(f'Parabéns {name_user}, você ganhou o sorteio!')
        else:
            print(f'Infelizmente você errou, o numero erá {get_codiguin}')

        tentativas -= 1
        regame = input(f'Deseja jogar o jogo novamente? digite "s" para sim')
        if tentativas < 0 and regame != 's':
